...
function normalizePcNote(note, categories = null, fallbackOrder = 0) {
  ...
  if (!content && checklistItems.length === 0) return null
...
function textFromChecklistItems(items) {
  return normalizeChecklistItems(items).map(item => item.text).join('\n')
}
...
function noteSearchText(note) {
  return [
    ...
    note.title || '',
    note.body || '',
    ...normalizeChecklistItems(note.checklistItems).map(item => item.text)
  ].join('\n').toLowerCase()
}
...
function normalizePcNoteOrders(notes) {
  const sortedNotes = sortedNotesForDisplay(notes)
...
function knownNoteCategoryIdForData(categoryId, categories = null) {
  ...
  if (normalizedId === ALL_NOTE_CATEGORY_ID) return null
  ...
  return ids.has(normalizedId) ? normalizedId : null
}
...
function normalizePcNotes(notes, categories = null) {
  ...
  return normalizePcNoteOrders(normalizedNotes)
}

function defaultPcNotes() {
  try {
    const raw = localStorage.getItem(PC_NOTES_STORAGE_KEY)
    return normalizePcNotes(raw ? JSON.parse(raw) : [], defaultNoteCategories())
  } catch {
    return []
  }
}
...
function plannerSettingsPayload(settings = {}) {
  const rawSettings = isPlainObject(settings) ? settings : {}
  ...
  const noteCategories = normalizeNoteCategories(rawNoteCategories)
  ...
  return {
    ...
    pcNotes: normalizePcNotes(rawPcNotes, noteCategories),
    noteCategories
  }
}
...
export default function App() {
  ...
  const pcNoteModalPointerStartedInsideRef = useRef(false)
  ...
  useEffect(() => {
    ...
    function handleNoteCardControlsOutsidePointerDown(e) {
      ...
      if (e.target.closest?.('.note-category-control-zone')) return

      setOpenNoteCardPaletteId(null)
      setOpenNoteCardMenuId(null)
      setOpenNoteCardMoveMenuId(null)
  ...
  }, [openNoteCardMoveMenuId, openNoteCardPaletteId, openNoteCardMenuId, openNoteCategoryMenuId])

  useEffect(() => {
    if (!noteToastMessage) return undefined
  ...
  const filteredPcNotes = useMemo(() => {
    ...
    const searchedNotes = query
      ? categoryFilteredNotes.filter(note => noteSearchText(note).includes(query))
      : categoryFilteredNotes

    return sortedNotesForDisplay(searchedNotes)
  }, [effectiveSelectedNoteCategoryId, noteCategoryIds, pcNoteSearch, pcNotes])
  const pinnedPcNotes = filteredPcNotes.filter(note => note.isPinned)
  const unpinnedPcNotes = filteredPcNotes.filter(note => !note.isPinned)
  ...
  function deleteNoteCategory(categoryId) {
    if (PROTECTED_NOTE_CATEGORY_IDS.has(categoryId)) return
  ...
  function movePcNoteToCategory(noteId, categoryId) {
    const nextCategoryId = normalizeKnownNoteCategoryId(categoryId)
    const note = pcNotes.find(item => item.id === noteId)
    if (!note || normalizeKnownNoteCategoryId(note.categoryId) === nextCategoryId) {
      setOpenNoteCardMoveMenuId(null)
      setOpenNoteCardMenuId(null)
      return
    }
    ...
    const timestamp = new Date().toISOString()
    const categoryName = noteCategoryName(noteCategories, nextCategoryId)

    saveUndoSnapshot()
    setPcNotes(prev => normalizePcNotes(prev.map(item => (
      item.id === noteId
        ? { ...item, categoryId: nextCategoryId, updatedAt: timestamp }
        : item
    ))))
    setOpenNoteCardMoveMenuId(null)
    setOpenNoteCardMenuId(null)
    setNoteToastMessage(`${categoryName}に移動しました`)
  }

  function togglePcNotePinned(noteId) {
    const timestamp = new Date().toISOString()
    saveUndoSnapshot()
    setPcNotes(prev => normalizePcNotes(prev.map(note => (
      note.id === noteId
  ...
  function togglePcNoteChecklistItem(noteId, itemId) {
    const timestamp = new Date().toISOString()
    saveUndoSnapshot()
    setPcNotes(prev => normalizePcNotes(prev.map(note => (
      note.id === noteId
        ? {
            ...note,
            checklistItems: normalizeChecklistItems(note.checklistItems).map(item => (
  ...
  function nextPcNoteOrder() {
    ...
    return Math.min(...pcNotes.map(note => Number(note.order) || 0)) - 1
  }
  ...
  function reorderPcNotes(draggedId, targetId, groupKey = 'all') {
    ...
    const originalIds = groupNotes.map(note => note.id).join('\n')
    const reorderedIds = reorderedGroup.map(note => note.id).join('\n')
    if (originalIds === reorderedIds) return

    saveUndoSnapshot()
    setPcNotes(prev => {
      const normalizedPrev = sortedNotesForDisplay(normalizePcNotes(prev, noteCategories))
      const groupIds = new Set(groupNotes.map(note => note.id))
      const firstGroupIndex = normalizedPrev.findIndex(note => groupIds.has(note.id))
      const notesOutsideGroup = normalizedPrev.filter(note => !groupIds.has(note.id))
      const insertAt = firstGroupIndex >= 0
        ? normalizedPrev.slice(0, firstGroupIndex).filter(note => !groupIds.has(note.id)).length
        : notesOutsideGroup.length
      const nextNotes = [
        ...notesOutsideGroup.slice(0, insertAt),
        ...reorderedGroup,
        ...notesOutsideGroup.slice(insertAt)
      ]

      return normalizePcNotes(nextNotes.map((note, index) => ({
        ...note,
        order: index
      })), noteCategories)
    })
  ...
  function markPcNoteDragOver(e, noteId, groupKey) {
    if (!draggedPcNoteId || draggedPcNoteId === noteId) return
    if (!pcNoteGroupNotes(groupKey).some(note => note.id === draggedPcNoteId)) return
    e.preventDefault()
    e.dataTransfer.dropEffect = 'move'
    setDragOverPcNoteEndGroup(null)
  ...
  function startPcNotePointerDrag(e, noteId, groupKey) {
    ...
    function handlePointerMove(moveEvent) {
      ...
      if (targetCard && targetCard.dataset.pcNoteId !== state.noteId && targetCard.dataset.pcNoteGroup === state.groupKey) {
        setDragOverPcNoteId(targetCard.dataset.pcNoteId)
        setDragOverPcNoteEndGroup(null)
      } else if (targetEnd && targetEnd.dataset.pcNoteDropEndGroup === state.groupKey) {
        setDragOverPcNoteId(null)
    ...
    function handlePointerUp(upEvent) {
      ...
      const targetCard = hovered?.closest?.('[data-pc-note-id]')
      const targetEnd = hovered?.closest?.('[data-pc-note-drop-end-group]')
      const isSameGroupCard = targetCard?.dataset.pcNoteGroup === state.groupKey
      const isSameGroupEnd = targetEnd?.dataset.pcNoteDropEndGroup === state.groupKey
      if (!isSameGroupCard && !isSameGroupEnd) {
        clearPcNoteDragState()
        return
      }

      const targetId = isSameGroupCard && targetCard.dataset.pcNoteId !== state.noteId
        ? targetCard.dataset.pcNoteId
        : null
      reorderPcNotes(state.noteId, targetId, state.groupKey)
      clearPcNoteDragState()
    }
  ...
  function convertNoteDraftType(setDraft, nextType) {
    setDraft(prev => {
      const currentType = prev.type === 'checklist' ? 'checklist' : 'text'
      ...
      const checklistText = textFromChecklistItems(prev.checklistItems)
      return {
        ...prev,
        type: 'text',
        content: [prev.content, checklistText].filter(Boolean).join('\n'),
        checklistItems: []
      }
    })
  }
  ...
  function closePcNoteComposer() {
    if (!isPcNoteDraftEmpty(pcNoteDraft)) {
      const shouldDiscard = window.confirm('未保存のメモがあります。閉じてもよいですか？')
  ...
  function closePcNoteEditor() {
    ...
    pcNoteModalPointerStartedInsideRef.current = false
  }
  ...
  function savePcNoteDraft() {
    ...
    const type = pcNoteDraft.type === 'checklist' ? 'checklist' : 'text'
    const checklistItems = type === 'checklist'
      ? normalizeChecklistItems(pcNoteDraft.checklistItems)
      : []
  ...
  function savePcNoteEditDraft() {
    ...
    const type = pcNoteEditDraft.type === 'checklist' ? 'checklist' : 'text'
    const checklistItems = type === 'checklist'
      ? normalizeChecklistItems(pcNoteEditDraft.checklistItems)
      : []
  ...
  function handlePcNoteBackdropPointerDown(e) {
    pcNoteModalPointerStartedInsideRef.current = e.target !== e.currentTarget
  }

  function handlePcNoteBackdropClick(e) {
    ...
    if (pcNoteModalPointerStartedInsideRef.current || hasActiveTextSelection()) {
      pcNoteModalPointerStartedInsideRef.current = false
      return
    }
  ...
  function deletePcNote(id) {
    ...
    setOpenNoteCardMoveMenuId(prev => (prev === id ? null : prev))
  ...
  function toggleNoteCardMenu(e, id) {
    e.stopPropagation()
    setOpenNoteCardPaletteId(null)
    if (openNoteCardMenuId === id) {
      setOpenNoteCardMenuId(null)
      setOpenNoteCardMoveMenuId(null)
      return
    }
    setOpenNoteCardMoveMenuId(null)
    setOpenNoteCardMenuId(id)
  }

  function toggleNoteCategoryMenu(e, id) {
    e.stopPropagation()
    setOpenNoteCategoryMenuId(prev => (prev === id ? null : id))
  }
      ...
      <aside className={isMobileCategoryMenu ? 'note-category-sidebar mobile' : 'note-category-sidebar'}>
        ...
        <div className="note-category-list">
          {noteCategories.map(category => {
            ...
            const canDelete = !PROTECTED_NOTE_CATEGORY_IDS.has(category.id)
            const isCategoryMenuOpen = openNoteCategoryMenuId === category.id

            return (
              <div
                key={category.id}
                className={`note-category-row note-category-control-zone ${isSelected ? 'selected' : ''}`}
              >
                <button
                  type="button"
                ...
                {canDelete && (
                  <div className="note-category-menu-area">
                    <button
                      type="button"
                      className="note-category-kebab"
                      onClick={e => toggleNoteCategoryMenu(e, category.id)}
                    ...
                    {isCategoryMenuOpen && (
                      <div className="note-category-menu" role="menu">
                        <button
                          type="button"
                          role="menuitem"
  ...
  function renderPcNoteCard(note, groupKey = 'all', mode = 'pc') {
    ...
    const isMoveMenuOpen = openNoteCardMoveMenuId === note.id
    const checklistItems = normalizeChecklistItems(note.checklistItems)
    ...
    return (
      <article
        ...
        onDragOver={e => markPcNoteDragOver(e, note.id, groupKey)}
        ...
        onClick={e => {
          ...
          openPcNoteEditor(note, mode)
        }}
      >
        <button
          type="button"
          className={`pc-note-pin-button pc-note-card-control-zone ${note.isPinned ? 'active' : ''}`}
          onClick={e => {
            e.stopPropagation()
            togglePcNotePinned(note.id)
          }}
        ...
        {isChecklist ? (
          <ul className="pc-note-checklist-preview" onClick={e => e.stopPropagation()}>
              ...
              <li key={item.id} className={item.checked ? 'checked' : ''}>
                <label>
                  <input
                    type="checkbox"
                    checked={item.checked}
                    onChange={() => togglePcNoteChecklistItem(note.id, item.id)}
                  />
                  <span>{item.text}</span>
                </label>
              </li>
        ...
        {isChecklist && noteParts.body && <p>{noteParts.body}</p>}
            ...
            {isCardMenuOpen && (
              <div className="pc-note-card-menu" role="menu">
                <button
                  type="button"
                  role="menuitem"
                  className="pc-note-card-menu-neutral"
                  onClick={() => setOpenNoteCardMoveMenuId(prev => (prev === note.id ? null : note.id))}
                >
                  カテゴリ移動
                </button>
                {isMoveMenuOpen && (
  ...
  function renderPcNoteGrid(notes, groupKey, label, mode = 'pc') {
    return (
      <div className="pc-notes-masonry" aria-label={label}>
        {notes.map(note => renderPcNoteCard(note, groupKey, mode))}
        <div
  ...
  function renderNotesWorkspace(mode = 'pc') {
    ...
    return (
      <section className={isMobileNotes ? 'pc-notes-shell mobile-notes-shell' : 'pc-notes-shell'}>
        ...
        ) : pinnedPcNotes.length > 0 ? (
          <>
            <section className="pc-note-group">
              <h3>ピン留め</h3>
              {renderPcNoteGrid(pinnedPcNotes, 'pinned', 'ピン留めメモ一覧', mode)}
            </section>
            {unpinnedPcNotes.length > 0 && (
              <section className="pc-note-group">
                <h3>その他</h3>
                {renderPcNoteGrid(unpinnedPcNotes, 'unpinned', 'その他のメモ一覧', mode)}
              </section>
            )}
          </>
        ) : (
          renderPcNoteGrid(filteredPcNotes, 'all', 'メモ一覧', mode)
        )}
  ...
  return (
    <div className="app-root">
      ...
      {editingPcNoteId && (
        <div
          className="pc-note-modal-backdrop"
          role="presentation"
          onPointerDown={handlePcNoteBackdropPointerDown}
          onClick={handlePcNoteBackdropClick}
        >
          <section
